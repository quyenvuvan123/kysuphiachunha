/**
 * Client-Side Heuristic MEP Domain Expert Fallback Engine
 * Provides instant, high-grade Vietnamese MEP engineering content,
 * 4-layer quotation audits, calendar schedules, and engineer consultation replies
 * if deployed to static hosting / Vercel without backend server or if backend is unreachable.
 */
...
export function generateFallbackContent(params: {
  ...
  const { format = "", topic = "Kỹ thuật MEP" } = params;
